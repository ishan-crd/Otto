/** Login / sign-up for the Buy-a-Prompt marketplace, served at /login. */
...
<style>
  :root{ --bg:#0A0A0B; --tx:#F2F1F6; --mut:rgba(242,241,246,0.55); --dim:rgba(242,241,246,0.34);
    --lav:#B3AAFF; --grn:#8FE3B4; --grn2:#A9EFC8; --card:rgba(255,255,255,0.04); --bd:rgba(255,255,255,0.08); }
  *{box-sizing:border-box}
  ...
  input,select{width:100%;height:46px;background:rgba(10,10,11,0.5);border:1px solid var(--bd);border-radius:12px;color:var(--tx);font-family:inherit;font-size:14.5px;padding:0 14px;outline:none}
  input:focus,select:focus{border-color:rgba(169,160,255,0.5)}
  ...
  .msg{font-size:12.5px;color:#FFB3AC;text-align:center;margin-top:12px;min-height:16px}
  .msg.ok{color:var(--grn2)}
  .quick{margin-top:18px;padding-top:18px;border-top:1px solid var(--bd)}
  .qlabel{font-size:10.5px;letter-spacing:0.08em;color:var(--dim);text-align:center;margin-bottom:10px}
  .qrow{display:flex;gap:10px}
  .qbtn{flex:1;height:44px;border-radius:12px;border:1px solid var(--bd);background:rgba(255,255,255,0.04);color:var(--tx);font-family:inherit;font-size:13px;cursor:pointer}
  .qbtn:hover{background:rgba(255,255,255,0.08)}
  .qbtn b{display:block;font-size:13px;margin-bottom:2px}
  .qbtn small{color:var(--dim);font-family:var(--mono);font-size:10.5px}
...
<div class="box">
  ...
  <div class="sub" id="sub">Sign in to buy and sell prompts over x402.</div>
  <div class="card">
    ...
    <label>EMAIL</label><input id="email" type="email" placeholder="you@otto.com" autocomplete="username" />
    <label>PASSWORD</label><input id="password" type="password" placeholder="••••••••" autocomplete="current-password" />
    <div id="signupRole" style="display:none">
      <label>I WANT TO</label>
      <select id="role"><option value="buyer">Buy prompts</option><option value="seller">Sell prompts (connect my Claude)</option></select>
    </div>
    <button class="btn" id="submit">Log in</button>
    <div class="msg" id="msg"></div>

    <div class="quick">
      <div class="qlabel">QUICK DEMO LOGIN</div>
      <div class="qrow">
        <button class="qbtn" id="qSeller"><b>Seller</b><small>seller@otto.com</small></button>
        <button class="qbtn" id="qBuyer"><b>Buyer</b><small>buyer@otto.com</small></button>
      </div>
    </div>
  </div>
</div>
...
function setMode(m){
  ...
  el('signupOnly').style.display=m==='signup'?'block':'none';
  el('signupRole').style.display=m==='signup'?'block':'none';
...
async function submit(){
  ...
  if(!email||!password){ el('msg').textContent='Enter email and password.'; return; }
  var btn=el('submit'); btn.disabled=true;
  var path=mode==='signup'?'/api/auth/signup':'/api/auth/login';
  var body=mode==='signup'?{ name:el('name').value.trim(), email:email, password:password, role:el('role').value }:{ email:email, password:password };
  try{
    ...
    setTimeout(function(){ location.href='/prompt'; }, 500);
...
function quick(email,password){ setMode('login'); el('email').value=email; el('password').value=password; submit(); }
el('qSeller').onclick=function(){ quick('seller@otto.com','seller'); };
el('qBuyer').onclick=function(){ quick('buyer@otto.com','buyer'); };

// already signed in? go straight in.
if(localStorage.getItem('otto_token')){
  fetch('/api/auth/me',{headers:{Authorization:'Bearer '+localStorage.getItem('otto_token')}}).then(function(r){ if(r.ok) location.href='/prompt'; });
}
</script>
</body>
</html>`;
